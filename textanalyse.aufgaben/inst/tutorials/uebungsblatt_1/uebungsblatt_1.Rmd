...
# custom functions
expand_variants <- function(base_forms)
  {
  variants <- unique(unlist(lapply(base_forms, function(x) {
    c(x, tolower(x), toupper(x), stringr::str_to_title(x))
  })))
  return(variants)
}

custom_question_text <- function(text, variants, correct_msg="Super, das ist richtig!", incorrect_msg="Leider nicht richtig.", allow_retry = TRUE, placeholder="Antwort eingeben...") {
  variants = expand_variants(variants)
  do.call(question_text, c(
    list(text),
    lapply(variants, answer, correct = TRUE),
    list(
      correct = correct_msg,
...
```{r results = FALSE, echo=TRUE}
"Es war einmal ein König" == "Es war einmal eine Königigin"
("Es war einmal ein König" == "Es war einmal eine Königigin") != "Ein schöner Frosch"
